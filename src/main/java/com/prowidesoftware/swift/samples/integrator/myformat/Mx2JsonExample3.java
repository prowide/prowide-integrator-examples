...
/**
 * Converts the same MX as {@link Mx2JsonExample1}, but with the FOREACH sheet of
 * {@code /myformat/mx2json.xls}.
 *
 * <p>Wrapping a selector in {@code FOREACH(...)} makes the engine iterate the repetitive element
 * and emit one target entry per occurrence, instead of folding them into a single value. Run this
 * next to {@link Mx2JsonExample1} on the same input to see the difference: the two credit transfers
 * of the second payment keep their own {@code InstdAmt} and {@code IBAN} here, while the SIMPLE
 * sheet concatenates them.
 *
 * <p>A {@code FOREACH} can also carry a second, relative selector to pull a value from an ancestor
 * of the iterated element, as in {@code FOREACH(.../CdtTrfTxInf, ../DbtrAcct/Id/IBAN)}.
 *
 * <p>Requires the Prowide Integrator MyFormat module.
 */
public class Mx2JsonExample3 {
    ...
    public static void main(String[] args) {
        ...
        MappingTableExcelLoader loader = new MappingTableExcelLoader(
                Objects.requireNonNull(Mx2JsonExample3.class.getResourceAsStream("/myformat/mx2json.xls")));
