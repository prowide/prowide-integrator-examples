...
Swift: fin.515
Sender:		RBDSJPJTBXXX
		RBC CAPITAL MARKETS (JAPAN) LTD., TOKYO (JAPAN)
Receiver:	BOJPJPJTXMF1
		BANK OF JAPAN - (FOR JAPAN'S RESERVES USE ONLY) (JAPAN)
------------------------- Message Text -------------------------
...
public class ExpandedPrintoutExample {
	...
	public static void main(String[] args) {
		...
		MT103 mt103 = MT103.parse("{1:F01BACOARB1A0B20000000000}{2:I103ADRBNL21XXXXN}{4:\n" +
				":20:REFERENCE\n" + 
				":23B:CRED\n" + 
		...
		MT515 mt515 = MT515.parse("{1:F01RBDSJPJTBXXX0000000000}{2:I515BOJPJPJTXMF1N}{4:\n" +
				":16R:GENL\n" +
				":20C::SEME//M1999999.1\n" +
				":23G:NEWM\n" +
				":22F::TRTR//TRAD\n" +
		...
		MT940 mt940 = MT940.parse("{1:F01BACOARB1A0B20000000000}{2:I940BSCHGB2LXEQUN}{4:\n" +
	            ":20:123456\n"+
				":25:123-304958\n"+
				":28C:123/1\n"+
		...
		MT202 mt202 = MT202.parse("{1:F01BICFOOYYAXXX8628453424}{2:O2021300050901IRVTLULXALTA06556102830509011300N}{4:\n" + 
		":20:RFSAMPPGN0031091\n" + 
		":21:RFSAMPPGN0031091\n" + 
	...
	private static String expanded(final SwiftMessage msg) {
		...
		out.append("------------------------- Message Header -----------------------").append("\n");
		out.append("Swift: ").append(msg.getMtId().id()).append("\n");
		...
		/*
		 * iterate message fields detectign sequences start/end
		 */
		for (Tag tag : msg.getBlock4().getTags()) {
			if ("16R".equals(tag.getName())) {
				 ...
				 */
				out.append("-------- ").append(currentSequence).append(" - ").append(info.sequenceName(currentSequence)).append(" --------\n");
				 ...
				 * print sequence name and description
				 */
				out.append("-------- ").append(currentSequence).append(" - ").append(info.sequenceName(tag.getLetterOption())).append(" --------\n");
			} else {
				/*
				...
				Field f = tag.asField();
				out.append(f.getName()).append(": ").append(fieldLabel).append("\n");
				for (int component=1; component<=f.getComponents().size(); component++) {
					...
					if (value != null) {
						...
						if (StringUtils.equals("BIC", label)) {
							 ...
							 */
							out.append(value);
							final String name = expandBIC(repo, value.trim());
							if (name != null) {
	 ...
	 * returns a strign with the institution name and, if present,
